...
PG.prototype.connect = function (callback) {
    if (this.connected) {
        console.error('jugglingdb-postgres: connect() called more than once.');
        return callback();
    }
...
PG.prototype.toFields = function (model, data, forCreate) {
    ...
    if(forCreate){
        ...
        Object.keys(props).forEach(function (key) {
          keyProps = props[key]
          column = keyProps.name || key
          if(data.hasOwnProperty(column)) {
              if (key === 'id') return;
              columns.push('"' + column + '"');
              fields.push(this.toDatabase(keyProps, data[column]));
          }
      }.bind(this));
      ...
      Object.keys(props).forEach(function (key) {
          ...
          if(data.hasOwnProperty(column)) {
              fields.push(
                  '"' + column + '" = ' + this.toDatabase(keyProps, data[column]));
          }
      }.bind(this));
      return fields.join(',');
    }
...
PG.prototype.getColumns = function(model){
    ...
    return '"' + Object.keys(props)
        .map( function(key) { return props[key].name || key } )
        .join('", "') + '"';
}


PG.prototype.all = function all(model, filter, callback) {
    this.query('SELECT ' + this.getColumns(model) +  '  FROM ' + this.tableEscaped(model) + ' ' + this.toFilter(model, filter, true), function (err, data) {
        if (err) {
            return callback(err, []);
        }
...
PG.prototype.toFilter = function (model, filter, translated) {
    ...
    if (translated===true) {
        ...
        Object.keys(props).forEach( function(key) {
            ...
            if (typeof prop.name == 'string') {
                translatedProps[prop.name] = prop;
            } else {
                translatedProps[key] = prop;
            }
        });
...
PG.prototype.propertiesSQL = function (model) {
    ...
    Object.keys(props).forEach(function (prop) {
        if (prop === 'id') return;
        column = props[prop].name || prop
        sql.push('"' + column + '" ' + self.propertySettingsSQL(model, prop));
    });
    return sql.join(',\n  ');
...
PG.prototype.propertySettingsSQL = function (model, propName) {
    ...
    if(!propertyIsUnique.call(self, model, propName)) result = result + 'UNIQUE ';
    return result;
};
